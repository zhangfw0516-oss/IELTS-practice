...
### 2026-09-04 自适应背词与复习逻辑

- 首次学习改为每组 10 词的两次间隔回忆；“会了”和“熟练”分开记录，完成本组只代表开始记忆，不再直接显示为全面掌握。
- 新词按约 10 分钟、12 小时进入当日巩固，再按 1、3、7、14、30 天进入跨日复习；“困难”缩短间隔，“忘了”会回到 10 分钟再学习，但保留历史记录。
- 同一词在一组内最多失败三次，之后移入后续复习，避免无限循环。累计遗忘四次会标为“顽固词”，便于重点处理。
- 阅读摘词默认练“看词识义”，不强制拼写；听力错词默认先听真人英音再拼写；普通词表保留识义与主动拼写。
- 待复习达到 20 个时减少一半新词，达到 50 个时暂停新词，优先清理复习积压。
- 新调度字段与原 SM-2 字段并存，旧学习记录无需清空；10 分钟、12 小时等日内间隔按小数天原样持久化。

### 2026-09-03 改版修复与云同步注意事项

- 修复背词重复计数、答错/提示后的评分、两阶段学习、复习专用队列、计时暂停和断点恢复。历史统计中的虚高数值无法可靠还原，本次不清空已有记录。
- 云同步默认手动；「立即同步」先合并云端，再用事务上传。自动同步需主动开启。相同单词保留稳定本地 ID，冲突按记录时间处理；时钟差异仍可能影响选择。合并以保留记录为主，不提供跨设备删除传播。
- 云端格式升级为 v3；各设备应刷新后再同步，避免旧页面写入旧格式。上传前会检查 900 KB 安全容量，超限保留本地与上一份云备份并提示导出；尚未实现分片备份。
- 首次同步会绑定此浏览器本地数据的账号。退出账号不会删除本地记录；切换账号前请先导出并清除本地数据，或使用独立浏览器配置，以免把不同同学的记录混在一起。
- 学习记录仍以浏览器本地数据为基础。背词统计和断点暂保留旧 localStorage 键以兼容现有版本，通过显式同步桥接；不是所有数据都已迁入 AppData V2。请继续定期导出本地备份。
- 新账号密码至少 8 位；旧账号登录保持兼容。昵称账号没有真实收件邮箱，不能邮件找回；新用户可用真实邮箱注册并使用找回按钮。
- 页面文本、导入字段和词典内容作转义处理；词典来源按实际接口标示，不将非 Oxford/Cambridge 内容冒充其原文。

#### Firebase 权限规则必须单独核验

`firestore.rules` 是按用户 UID 隔离的规则模板，`firebase.json` 是部署入口。**提交到 GitHub/部署 Pages 不会自动更新 Firebase 线上规则，也不能证明线上账号隔离已生效。**

维护者需先备份 Firebase 控制台现有规则，确认没有其他业务依赖，然后通过控制台或已授权的 Firebase CLI 部署到正确项目，并用两个测试账号验证：自己可读写自己的 `users/{uid}/sync/latest`，未登录及其他用户不可读写。CLI 示例（请核对项目）：

```bash
firebase deploy --only firestore:rules --project ielts-57ef8
```

本轮自动化测试模拟云服务，不会使用真实账号、修改同学记录或部署后台规则。
